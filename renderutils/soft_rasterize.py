...
soft_rasterize_cuda = load(name="soft_rasterize_cuda",
                             sources=["../renderutils/cuda/soft_rasterize_cuda.cpp",
                                      "../renderutils/cuda/soft_rasterize_cuda_kernel.cu"],
                                      verbose=True,
                                      with_cuda=True,
...
class SoftRasterizeFunction(Function):
    ...
    def forward(
        ...
        ctx.dist_eps = np.log(1.0 / dist_eps - 1.0)
        ...
        faces_info = torch.zeros(
            ctx.batch_size, ctx.num_faces, 9 * 3, device=ctx.device
        )
        aggrs_info = torch.zeros(
            ctx.batch_size, 2, ctx.image_size, ctx.image_size, device=ctx.device
        )

        soft_colors = (
            torch.FloatTensor(ctx.batch_size, 4, ctx.image_size, ctx.image_size)
            .fill_(1.0)
            .to(device=ctx.device)
        )
        soft_colors[:, 0, :, :] *= background_color[0]
        soft_colors[:, 1, :, :] *= background_color[1]
        soft_colors[:, 2, :, :] *= background_color[2]

        faces_info, aggrs_info, soft_colors = soft_rasterize_cuda.forward(
            face_vertices,
            textures,
            faces_info,
            aggrs_info,
        ...
        ctx.save_for_backward(
            face_vertices, textures, soft_colors, faces_info, aggrs_info
        )
        return soft_colors
    ...
    def backward(ctx, grad_soft_colors):
        ...
        grad_faces = (
            torch.zeros_like(face_vertices, dtype=torch.float32)
            .to(ctx.device)
            .contiguous()
        )
        grad_textures = (
            torch.zeros_like(textures, dtype=torch.float32).to(ctx.device).contiguous()
        )
        grad_soft_colors = grad_soft_colors.contiguous()

        grad_faces, grad_textures = soft_rasterize_cuda.backward(
            face_vertices,
            textures,
            soft_colors,
            faces_info,
...
def soft_rasterize(
    ...
    return SoftRasterizeFunction.apply(
        face_vertices,
        textures,
        image_size,
        background_color,
