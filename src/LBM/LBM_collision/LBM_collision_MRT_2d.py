...
class LBMCollisionMRT2d(LBMCollision2d):
    ...
    def get_s_by_KBC(
        ...
        if need_centering:
            p = torch.Tensor([[0, 0, 0], [1, 1, 1], [2, 2, 2]]).reshape(1, 1, 3, 3, *([1] * dim)).to(self.device).to(torch.int32)
            q = torch.Tensor([[0, 1, 2], [0, 1, 2], [0, 1, 2]]).reshape(1, 1, 3, 3, *([1] * dim)).to(self.device).to(torch.int32)
            vp = c * torch.pow((self._e[:, :, 0:1, ...] - vel[:, 0:1, ...]).unsqueeze(2), p)  # [B, Q, 3, 3, *res]
            vq = c * torch.pow((self._e[:, :, 1:2, ...] - vel[:, 1:2, ...]).unsqueeze(2), q)  # [B, Q, 3, 3, *res]
            vp_vq = vp * vq  # [B, Q, 3, 3, *res]
            moments_flux = (vp_vq * (f / rho).unsqueeze(2).unsqueeze(2)).sum(dim=1)  # [B, 3, 3, *res]
        else:
            moments_flux = (self.C_mat * (f / rho).unsqueeze(2).unsqueeze(2)).sum(dim=1)  # [B, 3, 3, *res]
        ...
        if not is_KBC:
            ...
            s[:, 0, ...] += rho[:, 0, ...] * (A)
            sigma_ = 1
            s[:, 1, ...] += 0.5 * rho[:, 0, ...] * (sigma_ * (vel[:, 0, ...] - Qxyy) - A)
            sigma_ = -1
            s[:, 3, ...] += 0.5 * rho[:, 0, ...] * (sigma_ * (vel[:, 0, ...] - Qxyy) - A)
            lambda_ = 1
            s[:, 2, ...] += 0.5 * rho[:, 0, ...] * (lambda_ * (vel[:, 1, ...] - Qxxy) - A)
            lambda_ = -1
            s[:, 4, ...] += 0.5 * rho[:, 0, ...] * (lambda_ * (vel[:, 1, ...] - Qxxy) - A)
            sigma_, lambda_ = 1, 1
    ...
    def collision(
        ...
        ds = (
            self.get_s_by_KBC(dx=dx, dt=dt, f=f, rho=rho, vel=vel, KBC_type=KBC_type) -
            self.get_s_by_KBC(dx=dx, dt=dt, f=feq, rho=rho, vel=vel, KBC_type=KBC_type)
        )
        dh = (f - feq) - ds

        beta = 0.5 / self._tau
        gamma = 1.0 / beta - (2.0 - 1.0 / beta) * (ds * dh / feq).sum(dim=1).unsqueeze(1) / (dh * dh / feq).sum(dim=1).unsqueeze(1)
